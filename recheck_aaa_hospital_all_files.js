...
function recheckAAAFiles() {
    const aaaDir = '/Users/arbiter/Dropbox/!Umysql_PVM/LOINC/saved_mappings/AAA_Hospital';

    // Get all JSON files
    const allFiles = fs.readdirSync(aaaDir)
        .filter(file => file.endsWith('.json'))
        .sort();

    console.log(`=== 萬芳醫院完整檔案檢查 ===`);
    ...
    allFiles.forEach((file, index) => {
        try {
            const filePath = path.join(aaaDir, file);
    ...
    if (missingRanks.length > 0) {
        ...
        console.log(`詳細缺漏: ${missingRanks.join(', ')}`);
    } else {
        console.log('無缺漏排名');
    }
    ...
    if (ranksOver200.length > 0) {
        ...
        const itemsOver200 = allMappings.filter(m => m.itemRank > 200).sort((a, b) => a.itemRank - b.itemRank);
        itemsOver200.forEach(item => {
    ...
    if (noRankItems.length > 0) {
        ...
        noRankItems.forEach((item, index) => {
            console.log(`  ${index + 1}. ${item.labItemName} (${item.labItemId}) - 排名: ${item.itemRank} - ${item.filename}`);
        });
    }
    ...
    const completeAnalysis = {
        summary: {
            ...
            ranksIn200Count: validRanks.filter(r => r >= 1 && r <= 200).length
        },
    ...
    fs.writeFileSync('/Users/arbiter/Dropbox/!Umysql_PVM/LOINC/aaa_hospital_complete_analysis.json', JSON.stringify(completeAnalysis, null, 2));
    ...
    fs.writeFileSync('/Users/arbiter/Dropbox/!Umysql_PVM/LOINC/aaa_hospital_mappings_corrected.json', JSON.stringify(finalMappings, null, 2));

    console.log(`\n=== 最終結果 ===`);
    console.log(`校正後的mapping數量: ${finalMappings.length}`);
    console.log(`1-200範圍內的項目: ${finalMappings.filter(m => m.itemRank >= 1 && m.itemRank <= 200).length}`);
    console.log(`檔案已保存: aaa_hospital_mappings_corrected.json`);
    console.log(`完整分析已保存: aaa_hospital_complete_analysis.json`);
